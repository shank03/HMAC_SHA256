...
private:
    ...
    private:
        ...
        static constexpr std::array<uint32_t, SHA256_BLOCK_SIZE> K = {
        ...
        static uint32_t rot_r(uint32_t x, uint32_t n) { return (x >> n) | (x << (32 - n)); }

        static uint32_t choose(uint32_t e, uint32_t f, uint32_t g) { return (e & f) ^ (~e & g); }

        static uint32_t majority(uint32_t a, uint32_t b, uint32_t c) { return (a & (b | c)) | (b & c); }

        static uint32_t sig0(uint32_t x) { return SHA256::rot_r(x, 7) ^ SHA256::rot_r(x, 18) ^ (x >> 3); }

        static uint32_t sig1(uint32_t x) { return SHA256::rot_r(x, 17) ^ SHA256::rot_r(x, 19) ^ (x >> 10); }
        ...
        void pad() {
            ...
            m_data[63] = (m_bit_len & 0xFF);
            m_data[62] = (m_bit_len >> 8) & 0xFF;
            m_data[61] = (m_bit_len >> 16) & 0xFF;
            m_data[60] = (m_bit_len >> 24) & 0xFF;
            m_data[59] = (m_bit_len >> 32) & 0xFF;
            m_data[58] = (m_bit_len >> 40) & 0xFF;
            m_data[57] = (m_bit_len >> 48) & 0xFF;
